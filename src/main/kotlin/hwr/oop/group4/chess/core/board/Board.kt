...
class Board {
    ...
    private fun generateFirstFieldOfRank(): Field {
        ...
        for (currentRank in 1..8) {
            val currentRankStart = generateRank(currentRank, previousRankStart, allFields)
            if (currentRank == 1) {
    ...
    private fun generateRank(rank: Int, previousRankStart: Field?, allFields: MutableMap<Location, Field>): Field {
    ...
    fun getField(location: Location): Field {
        return fields[location] ?: throw IllegalArgumentException("No field at ${location.description}")
    }
